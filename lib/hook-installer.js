...
// Task completion rides on the Stop hook, not on the idle_prompt notification.
// idle_prompt is emitted from the terminal REPL only, so it never arrives when
// Claude Code runs in an editor panel, and even in a terminal it is suppressed
// by any keypress after the turn ends. Stop is raised by the query loop itself,
// which makes it the only task-complete signal that works in every host.
